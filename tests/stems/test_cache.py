...
def test_save_and_load_demucs():
    onsets = StemOnsets(vocal_first_onset=55.2, drum_first_onset=4.1, bass_first_onset=15.8, other_first_onset=4.0)
    ...
    assert loaded_onsets.bass_first_onset == 15.8
    assert loaded_onsets.other_first_onset == 4.0


def test_save_and_load_librosa():
    onsets = StemOnsets(vocal_first_onset=1.0, drum_first_onset=2.0, bass_first_onset=3.0, other_first_onset=4.0)
...
def test_both_slots_coexist():
    hq_onsets = StemOnsets(vocal_first_onset=10.0)
    lq_onsets = StemOnsets(vocal_first_onset=1.0)
...
def test_save_and_load_with_none_values():
    onsets = StemOnsets(vocal_first_onset=None, drum_first_onset=None, bass_first_onset=None, other_first_onset=None)
...
def test_list_entries_populated():
    stems_cache.save("/music/a.mp3", StemOnsets(vocal_first_onset=1.0), "demucs")
    stems_cache.save("/music/b.mp3", StemOnsets(vocal_first_onset=2.0), "librosa")
...
def test_list_entries_hq_field():
    stems_cache.save("/music/a.mp3", StemOnsets(vocal_first_onset=1.0), "demucs")
    stems_cache.save("/music/a.mp3", StemOnsets(vocal_first_onset=0.5), "librosa")
    entries = stems_cache.list_entries()
    assert len(entries) == 2
